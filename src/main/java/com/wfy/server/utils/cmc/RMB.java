...
public class RMB {
    ...
    private static final String GRADE_UNIT = "仟万亿兆";
    private static final String DOT_UNIT = "角分厘";
...
*/
    ...
    private static String dotToBig(String dotPart) {
        ...
        for (int i = 0; i < dotPart.length() && i < 3; i++) {
            ...
            if ((num = Integer.parseInt(dotPart.substring(i, i + 1))) != 0)
                strRet += NUM.substring(num, num + 1) + DOT_UNIT.substring(i, i + 1);
        }
        return strRet;
    }
        ...
        for (int i = grade; i >= 1; i--) {
            ...
            if (i > 1) //末位不加单位
                ...
                if (getSubUnit(strTmp).equals("零零零零")) {
                    result += "零" + GRADE_UNIT.substring(i - 1, i);
                } else {
                    result += GRADE_UNIT.substring(i - 1, i);
                }
        }
        return result;
    }
    ...
    public static double fen2Yuan(Long amount){
        return BigDecimalUtil.div(amount, 100, 2);
    }

    public static String fen2YuanStr(Long amount){
        NumberFormat nf = NumberFormat.getInstance();
        nf.setMaximumFractionDigits(6);
        nf.setMinimumFractionDigits(2);
        return nf.format(fen2Yuan(amount));
    }

    public static void main(String[] args) {
        System.out.println(RMB.toBigAmt(0.0));
        System.out.println(RMB.toBigAmt(100));
        System.out.println(RMB.toBigAmt(10001.001));
        System.out.println(RMB.toBigAmt(12345678));
        System.out.println(RMB.toBigAmt(10052345.00D));
        System.out.println(RMB.toBigAmt(123456789.00D));
        ...
        System.out.println("---------------------------");
        System.out.println(fen2Yuan(500800L));
        System.out.println(fen2YuanStr(500800L));
